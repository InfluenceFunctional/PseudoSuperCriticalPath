...
def main():
    ...
    for i in range(1, number_steps // number_restart_steps + 1):
        ...
        with (open(f"../generate_restart_files_lambda//create_atoms.txt", "r") as read,
              open(f"/lambda_{current_lambda}/coeffs.txt", "w") as write):
            for line in read:
        ...
        with (open("run_MD.lmp", "r") as read,
              open(f"/lambda_{current_lambda}/run_MD.lmp", "w") as write):
            text = read.read()
        ...
        copy("sub_job.slurm", f"/lambda_{current_lambda}/sub_job.slurm")
