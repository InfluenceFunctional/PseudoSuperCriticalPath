...
ChangeBoxMol::ChangeBoxMol(LAMMPS *lmp) : 
Command(lmp), m_proc(nullptr), m_total(nullptr), mr_proc(nullptr), rcm(nullptr) {}


ChangeBoxMol::~ChangeBoxMol() 
{
...
void ChangeBoxMol::command(int narg, char **arg)
{
  int i;
  ...
  if (isRemapTrue == true)
  {
      ...
      for (i = 0; i < atom->nlocal; i++)
        if (mask[i] & groupbit) {
          ...
          domain->unmap(x[i], image_old[i], unwrap);
      ...
      for (i = 0; i < atom->nlocal; i++)
        if (mask[i] & groupbit) {
          imol = molecule[i]-1;         
          domain->unmap(x[i], image_old[i], delta[i]);                    
          delta[i][0] -= rcm[imol][0];
          delta[i][1] -= rcm[imol][1];
          delta[i][2] -= rcm[imol][2];
          x[i][0] -= delta[i][0];
          x[i][1] -= delta[i][1];
          x[i][2] -= delta[i][2];                 
        }
...
void ChangeBoxMol::options(int narg, char **arg)
{
...
void ChangeBoxMol::save_box_state()
{
...
void ChangeBoxMol::volume_preserve(int dim1, int dim2, double oldvol)
{
...
void ChangeBoxMol::allocate(int nmolecules)
{
  memory->destroy(m_proc);
  memory->destroy(m_total);
  memory->destroy(mr_proc);
  ...
  memory->create(m_proc, nmolecules, "change_box_mol:m_proc");
  memory->create(m_total, nmolecules, "change_box_mol:m_total");
  memory->create(mr_proc, nmolecules, 3, "change_box_mol:mr_proc");
  memory->create(rcm, nmolecules, 3, "change_box_mol:rcm");
}
